                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            {{ $event->event_date->diffInDays($event->end_date) + 1 }}
                            days @else N/A @endif
                        </td>
                        <td>
                            {{ $event->capacity - $event->bookings->where('status', 'confirmed')->sum('number_of_tickets') }}
                        </td>
                        <td>
                            {{ $event->bookings->where('status', 'confirmed')->sum('number_of_tickets') }}
                        </td>
                        <td>
                            @if($event->event_date > now()) Not Started
                            @elseif($event->end_date && $event->end_date >
                            now())
                            {{ $event->end_date->copy()->endOfDay()->diffInDays(now()->startOfDay()) }}
                            days @elseif($event->end_date && $event->end_date <=
                            now()) Ended @else Ongoing @endif
                        </td>
                        <td>
                            <a
                                href="{{ route('organizer.events.details', $event->id) }}"
